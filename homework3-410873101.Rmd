...
```{r ans14}
badEmployers$lawContent[names(mostViolatedArticle) == badEmployers$law][[1]] -> mostViolatedArticle_content
...
factor是從1開始的整數，沒有0喔。
```{r ans21}
ordered(factor(table$`國文` , levels=table$`國文`[c(16:1)])) -> chinese
chinese
```
...
```{r ans24}
ntpu_eco[ntpu_eco %in% namelist] -> A
paste("恭喜" , A , "同學錄取國立臺北大學經濟系") -> congrats
congrats
```
