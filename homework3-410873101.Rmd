...
```{r ans13}
table_countsByArticle == max(table_countsByArticle) -> A
table_countsByArticle[A] -> mostViolatedArticle
...
```{r ans14}
table(badEmployers$lawContent) == max(table(badEmployers$lawContent)) -> B
names(table(badEmployers$lawContent)[B]) -> mostViolatedArticle_content
...
factor是從1開始的整數，沒有0喔。
```{r ans21}
chinese <- ordered(factor(table$`級分`+1))
chinese
```
...
```{r ans22}
str_detect(students10$`志願`,"北大") -> ntpu_namelist
ntpu_namelist
```
...
logical儲存的本質是甚麼?
```{r ans23}
pick_ntpueco <- students10$`志願` %in% "北大經濟"

pick_ntpueco1 <- students10$`國文`[pick_ntpueco] >= 12

pick_ntpueco2 <- 
  (students10$`英文`[pick_ntpueco][pick_ntpueco1] + students10$`數學`[pick_ntpueco][pick_ntpueco1]) >= 26

ntpu_eco <- students10$`姓名`[pick_ntpueco][pick_ntpueco1][pick_ntpueco2]
ntpu_eco
```
...
```{r ans24}
admit <- ntpu_eco %in% namelist
congrats <- "恭喜 陳依祈、林溺 同學錄取國立臺北大學經濟系"
class(congrats)
congrats
```
