...
tpe_highSchools <- factor(tpe_highSchools, levels = c("臺北市立第一女中","台北市立第一女子高級中學","中山女中","中山女高", "台北市立中山女中", "臺北市立中山女子高級中學","國立政大附中",
"國立政治大學附屬高級中學"))
tpe_highSchools
```
...
levels(tpe_highSchools)
factor1 <- c("北一女","北一女","北一女","北一女","北一女","北一女","北一女","北一女","北一女","北一女")
tpe_highSchools[680:689] <- factor1
tpe_highSchools
```
...
```{r ans22}
str_detect(data228$`附註`, "表冊名稱:臺灣省二二八事變正法及死亡人犯名冊") -> A
data228$`個人名稱`[A] -> victims
victims
```
...
```{r ans32}
years <- courseStructure$`學年` == 103
econ <- str_detect(courseStructure$`應修系級`, "經濟系3")
unique(courseStructure$`科目名稱`[years & econ]) -> econ101junior
econ101junior
```
...
```{r ans33}
year1 <- courseStructure$`學年` == 101
finance1 <- str_detect(courseStructure$`應修系級`, "金融系1") 
unique(courseStructure$`科目名稱`[year1 & finance1]) -> A

year2 <- courseStructure$`學年` == 102
finance2 <- str_detect(courseStructure$`應修系級`, "金融系2") 
unique(courseStructure$`科目名稱`[year2 & finance2]) -> B

year3 <- courseStructure$`學年` == 103
finance3 <- str_detect(courseStructure$`應修系級`, "金融系3") 
unique(courseStructure$`科目名稱`[year3 & finance3]) -> C

year4 <- courseStructure$`學年` == 104
finance4 <- str_detect(courseStructure$`應修系級`, "金融系4") 
unique(courseStructure$`科目名稱`[year4 & finance4]) -> D

coursesOffered <- list(
  A, B, C, D
)
names(coursesOffered) <- c("firstyear", "secondyear", "thirdyear", "fourthyear")
coursesOffered
```
